...
class FrequencyRecoveryVisualization:
    ...
    def create_visualization(self, data_file='hybrid_freq_imec_data.pkl'):
        ...
        for idx, agent_name in enumerate(['ALICE', 'BOB', 'CHARLIE']):
            ...
            # Add "RECOVERED" annotation if BER < 40%
            if withkey_decoded[agent_name]["ber"] < 40:
...
Decoding Results:
  ALICE: {nokey_decoded['ALICE']['ber']:.0f}% BER
  BOB: {nokey_decoded['BOB']['ber']:.0f}% BER
  CHARLIE: {nokey_decoded['CHARLIE']['ber']:.0f}% BER
...
Messages are VISIBLE
        ...
        for agent in ['ALICE', 'BOB', 'CHARLIE']:
            ...
            if withkey_decoded[agent]['accuracy'] > 55:
                print(" <- RECOVERED")
            else:
                print()
