...
class FrequencyiMECHybridEncoder:
    ...
    def apply_imec_obfuscation(self, freq_tokens, context):
        ...
        """
        print("\n" + "="*80)
        print("STAGE 2: iMEC OBFUSCATION (with encryption, 16-bit blocks)")
        print("="*80)
        ...
        # Apply iMEC encoding to UNIFORM ciphertext with 16-bit blocks
        print(f"\n✓ Applying iMEC with 16-bit blocks...")
        print(f"  Block size: 16 bits (65,536 possible values per block)")
        print(f"  Number of blocks: {len(ciphertext_bits) // 16}")
        ...
        # Store metadata INCLUDING encryption key and 16-bit block size
        metadata = {
            'n_freq_tokens': len(freq_tokens),
            'block_size_bits': 16,
            'n_blocks': len(ciphertext_bits) // 16,
    ...
    def encode_hybrid(self, context, messages, sequence_length=300):
        ...
        """
        print("\n" + "="*80)
        print("HYBRID ENCODER: FREQUENCY + iMEC (16-bit blocks)")
        print("="*80)
        ...
        with open('hybrid_freq_imec_data_16bit.pkl', 'wb') as f:
        ...
        print(f"✓ Using 16-bit blocks: {metadata['n_blocks']} blocks")
        print(f"✓ Saved to: hybrid_freq_imec_data_16bit.pkl")
